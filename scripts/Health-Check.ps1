...
if (!(Test-RedisPong -RedisCli $redisCli)) { throw 'Redis is not reachable on 127.0.0.1:6379.' }
$info = @{}
$persistence = Invoke-RedisCli -RedisCli $redisCli -Arguments @('-h', '127.0.0.1', '-p', '6379', 'INFO', 'PERSISTENCE')
if ($persistence.ExitCode -ne 0) { throw "Redis INFO PERSISTENCE failed after a successful PONG (exit code $($persistence.ExitCode))." }
$persistence.Output | ForEach-Object { if ($_ -match '^([^:]+):(.*)$') { $info[$matches[1]] = $matches[2] } }
$lastSave = Invoke-RedisCli -RedisCli $redisCli -Arguments @('-h', '127.0.0.1', '-p', '6379', 'LASTSAVE')
if ($lastSave.ExitCode -ne 0) { throw "Redis LASTSAVE failed after a successful PONG (exit code $($lastSave.ExitCode))." }
