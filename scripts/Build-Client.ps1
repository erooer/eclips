...
try {
    & $toolchain.Mxmlc '-source-path+=src' '-library-path+=libs' "-output=$output" '-locale=en_US' '-default-size=800,600' '-default-frame-rate=60' '-default-background-color=#000000' '-swf-version=15' '-target-player=15.0' '-optimize=true' '-use-direct-blit=true' '-keep-as3-metadata+=Inject' '-keep-as3-metadata+=Embed' '-keep-as3-metadata+=PostConstruct' '-keep-as3-metadata+=ArrayElementType' 'src\WebMain.as'
...
Write-Host "Built SWF: $output; synchronized runtime and deployable web copies ($($buildHash.Substring(0, 12)))"
