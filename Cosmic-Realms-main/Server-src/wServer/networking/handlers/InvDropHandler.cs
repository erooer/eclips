    ...
    class InvDropHandler : PacketHandlerBase<InvDrop>
    {
        ...
        private void Handle(Player player, ObjectSlot slot)
        {
            ...
            var lifetime = player.Owner.Id == wServer.realm.worlds.World.Nexus
                ? player.Manager.Config.serverSettings.nexusDroppedItemLifetimeMs
                : 1000 * 60;
            if (item.Soulbound || player.Client.Account.Admin)
            {
                container = new Container(player.Manager, soulBag, lifetime, true);
                container.BagOwners = new int[] { player.AccountId };
            }
            else
            {
                container = new Container(player.Manager, normBag, lifetime, true);
            }
            Log.InfoFormat("[NEXUS_DROP_TRACE] created item={0} world={1} lifetimeMs={2}", item.ObjectId, player.Owner.Name, lifetime);
