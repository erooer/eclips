    ...
    partial class Player
    {
        ...
        private bool CanConsume(Item item)
        {
            ...
            var affectsMana = item.ActivateEffects.Any(effect =>
                effect.Effect == ActivateEffects.Magic || effect.Effect == ActivateEffects.MagicNova);

            // A potion should not be spent when none of its restorative effects can
            // change the player's authoritative state.  Non-restorative consumables
            // (keys, marks, etc.) retain their existing activation behaviour.
        ...
        private bool PersistConsumableState()
        {
            ...
            {
                SaveToCharacter();
                var saved = Manager.Database.SaveCharacter(
                    Client.Account,
                    Client.Character,
                    FameCounter.ClassStats,
                    true).GetAwaiter().GetResult();
                Log.InfoFormat("[USEITEM_TRACE] character persistence player={0} char={1} saved={2}.",
                    Id, Client.Character.CharId, saved);
                return saved;
            }
            catch (Exception exception)
            {
                Log.ErrorFormat("[USEITEM_TRACE] character persistence failed player={0}: {1}", Id, exception);
                return false;
            }
        }

        private void RejectConsumableUse(Entity entity, int slot, string reason)
        {
            Log.WarnFormat("[USEITEM_TRACE] rejected player={0}: {1}", Id, reason);
            entity.ForceUpdate(slot);
            Client.SendPacket(new InvResult { Result = 1 });
        }

        public void UseItem(RealmTime time, int objId, int slot, int packetObjectType, Position pos, byte useType)
        {
            using (TimedLock.Lock(_useLock))
            {
                ...
                if (entity == null)
                {
                    Log.WarnFormat("[USEITEM_TRACE] rejected player={0}: container object {1} was not found in world {2}.", Id, objId, Owner.Name);
                    Client.SendPacket(new InvResult() { Result = 1 });
                    return;
                }
                ...
                {
                    Log.WarnFormat("[USEITEM_TRACE] rejected player={0}: attempted use from another player object {1}.", Id, objId);
                    Client.SendPacket(new InvResult() { Result = 1 });
                    return;
                }
                ...
                {
                    Log.WarnFormat("[USEITEM_TRACE] rejected player={0}: container distance {1:0.00} exceeds 3.", Id, this.Dist(entity));
                    Client.SendPacket(new InvResult() { Result = 1 });
                    return;
                }
                ...
                // get item
                var stack = Stacks.FirstOrDefault(candidate => candidate.Slot == slot);
                Item item = null;
                if (stack != null)
                {
                    if (stack.Count <= 0)
                    {
                        Log.WarnFormat("[USEITEM_TRACE] rejected player={0}: stack slot {1} is empty.", Id, slot);
                        return;
                    }
                ...
                if (item == null)
                {
                    if (container == null)
                    {
                        Log.WarnFormat("[USEITEM_TRACE] rejected player={0}: object {1} is not an inventory container.", Id, objId);
                        return;
                    }

                    if (slot < 0 || slot >= cInv.Length)
                    {
                        Log.WarnFormat("[USEITEM_TRACE] rejected player={0}: slot {1} is outside container inventory length {2}.", Id, slot, cInv.Length);
                        Client.SendPacket(new InvResult() { Result = 1 });
                        return;
                    }
                ...
                if (item == null)
                {
                    Log.WarnFormat("[USEITEM_TRACE] rejected player={0}: slot {1} contains no item.", Id, slot);
                    return;
                }

                Log.InfoFormat("[USEITEM_TRACE] resolved player={0} item={1} type=0x{2:x4} packetType=0x{3:x4} typeMatch={4} consumable={5} slot={6} container={7} useType={8}",
                    Id, item.ObjectId, item.ObjectType, packetObjectType, item.ObjectType == packetObjectType,
                    item.Consumable, slot, entity.GetType().Name, useType);

                // make sure not trading and trying to cunsume item
                if (tradeTarget != null && item.Consumable)
                {
                    Log.WarnFormat("[USEITEM_TRACE] rejected player={0}: consumables cannot be used while trading.", Id);
                    return;
                }

                if (MP < item.MpCost)
                {
                    Log.WarnFormat("[USEITEM_TRACE] rejected player={0}: MP {1} is below cost {2} for {3}.", Id, MP, item.MpCost, item.ObjectId);
                    Client.SendPacket(new InvResult() { Result = 1 });
                    return;
                }

                if (item.Consumable && !CanConsume(item))
                {
                    RejectConsumableUse(entity, slot,
                        $"{item.ObjectId} has no restorative effect to apply at the current health/mana state.");
                    return;
                }
                ...
                if (slot < cInv.Length)
                {
                    ...
                    if (item.Consumable)
                    {
                        ...
                        // Player inventory is stored in the character hash.  Commit it
                        // synchronously before activation so a reconnect cannot reload
                        // the pre-use item while a delayed continuation is still pending.
                        if (ReferenceEquals(container, this) && !PersistConsumableState())
                        {
                            Inventory.Revert(cInv);
                            SaveToCharacter();
                            RejectConsumableUse(entity, slot, "character inventory persistence did not commit; inventory was restored.");
                            return;
                        }

                        if (slotType > 0)
                            FameCounter.UseAbility();
                        else if (item.ActivateEffects.Any(eff => eff.Effect == ActivateEffects.Heal ||
                        ...
                        Log.InfoFormat("[USEITEM_TRACE] activating player={0} item={1} effects={2}.", Id, item.ObjectId, item.ActivateEffects.Count());
                ...
                {
                    // Stack slots are not part of Inventory.  Decrement only after
                    // validation, then persist the character count before activation.
                    if (item.Consumable)
                    {
                        stack.Pull();
                        if (!PersistConsumableState())
                        {
                            stack.Count++;
                            RejectConsumableUse(entity, slot, "potion-stack persistence did not commit; stack was restored.");
                            return;
                        }
                    }
                    FameCounter.DrinkPot();
        ...
        // Data-driven seal blast used by Eye of the Ominous. Damage is calculated
        // entirely on the server from the player's effective Wisdom at activation.
        private void AEOminousSealBlast(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            const int wisdomStatIndex = 7;
            var effectiveWisdom = Stats.Base[wisdomStatIndex] + Stats.Boost[wisdomStatIndex];
            var calculatedDamage = (long)eff.TotalDamage + ((long)eff.TotalDamage * effectiveWisdom / 20);
            var damage = (int)Math.Min(int.MaxValue, Math.Max(0L, calculatedDamage));

            this.AOE(eff.Radius, false, entity =>
            {
                var enemy = entity as Enemy;
                enemy?.Damage(this, time, damage, false);
            });
            ...
            Client?.SendPacket(new ShowEffect
            {
                ...
                Pos1 = new Position { X = eff.Radius, Y = 0 }
            }, PacketPriority.Low);
        }
        ...
        private void AEUnlockPortal(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            ...
            // The locked shell stays visible while its destination is prepared.  The
            // unlocked portal is not inserted until it has a registered world, so an
            // apparently open key portal can always reconnect immediately.
            uPortal.PlayerOpened = true;
            uPortal.Opener = Name;
            EventHandler<World> ready = null;
            ready = (sender, world) =>
            {
                uPortal.WorldInstanceSet -= ready;
                if (world == null)
                {
                    SendError("The portal could not be formed.");
                    return;
                }
                OpenUnlockedPortal(portal, uPortal, portalDesc, uPortalDesc, world, gameData);
            };
            uPortal.WorldInstanceSet += ready;
            if (uPortal.BeginDestinationPreparation(Manager, Client, "key use"))
                ready(uPortal, uPortal.WorldInstance);
        }

        private void OpenUnlockedPortal(Portal lockedPortal, Portal unlockedPortal, PortalDesc lockedDesc, PortalDesc unlockedDesc, World world, XmlData gameData)
        {
            var source = Owner;
            if (source == null)
                return;
            if (!lockedDesc.NexusPortal || !Manager.Monitor.RemovePortal(lockedPortal))
                source.LeaveWorld(lockedPortal);
            unlockedPortal.Move(lockedPortal.X, lockedPortal.Y);
            unlockedPortal.Name = unlockedDesc.DisplayId;
            var position = new Position { X = lockedPortal.X - .5f, Y = lockedPortal.Y - .5f };
            if (!unlockedDesc.NexusPortal || !Manager.Monitor.AddPortal(world.Id, unlockedPortal, position))
                source.EnterWorld(unlockedPortal);

            if (!unlockedDesc.NexusPortal)
            {
                var timeoutMs = gameData.Portals[unlockedPortal.ObjectType].Timeout * 1000;
                if (source.Id == World.Nexus)
                    timeoutMs = Manager.Config.serverSettings.nexusTemporaryPortalLifetimeMs;
                Log.InfoFormat("[NEXUS_PORTAL_TRACE] opened portal={0} type=0x{1:x4} world={2} lifetimeMs={3}", unlockedPortal.Id, unlockedPortal.ObjectType, source.Name, timeoutMs);
                source.Timers.Add(new WorldTimer(timeoutMs, (w, t) =>
                {
                    Log.InfoFormat("[NEXUS_PORTAL_TRACE] expired portal={0} type=0x{1:x4} world={2}", unlockedPortal.Id, unlockedPortal.ObjectType, w.Name);
                    w.LeaveWorld(unlockedPortal);
                }));
            }
            source.BroadcastPacket(new Notification
            {
                Color = new ARGB(0xFF00FF00), ObjectId = Id, Message = "Unlocked by " + Name
