...
namespace wServer.realm.entities
{
    ...
    public partial class Player : Character, IContainer, IPlayer
    {
        ...
        // 0xF920 was originally an ability. Persisted inventories retain only item
        // types, so relocate legacy copies that still occupy an incompatible slot.
        private void MigrateLegacyMantle()
        {
            const ushort mantleType = 0xF920;
            var inventoryItems = Inventory.GetItems();
            var inventoryLimit = HasBackpack ? inventoryItems.Length : Math.Min(12, inventoryItems.Length);

            for (var equipmentSlot = 0; equipmentSlot < 4 && equipmentSlot < inventoryItems.Length; equipmentSlot++)
            {
                var item = inventoryItems[equipmentSlot];
                if (item?.ObjectType != mantleType || SlotTypes[equipmentSlot] == item.SlotType)
                    continue;

                var destination = -1;
                for (var inventorySlot = 4; inventorySlot < inventoryLimit; inventorySlot++)
                ...
                if (destination < 0)
                {
                    SendError("Mantle of the Below remains in its legacy ability slot because your inventory is full. Free an inventory slot, then relog.");
                    continue;
                }

                inventoryItems[destination] = item;
                inventoryItems[equipmentSlot] = null;
                Inventory.SetItems(inventoryItems);
                SendInfo("Moved legacy Mantle of the Below from an incompatible equipment slot to your inventory.");
            }
        }
        ...
        {
            var sourcePortal = (Portal)portal;
            sourcePortal.WorldInstanceSet -= Reconnect;
            Log.InfoFormat("[DYNAMIC_PORTAL_TRACE] world-ready callback player={0} state={1} portal={2} type=0x{3:x4} destination={4}.",
                Id, Client.State, sourcePortal.Id, sourcePortal.ObjectType,
                world == null ? "<null>" : world.Name + " (" + world.Id + ")");

            if (world == null)
            {
                Log.WarnFormat("[DYNAMIC_PORTAL_TRACE] callback rejected player={0}: portal has no destination.", Id);
                SendError("Portal Not Implemented!");
            }
            else
            {
                Log.InfoFormat("[DYNAMIC_PORTAL_TRACE] issuing reconnect player={0} gameId={1} port=2050.", Id, world.Id);
                Client.Reconnect(new Reconnect()
                {
                    Host = "",
                    Port = 2050,
