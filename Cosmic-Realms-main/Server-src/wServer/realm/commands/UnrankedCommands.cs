...
namespace wServer.realm.commands
{
  ...
  /*  class NpeCommand : Command
    ...
    */
    ...
    class ArenaCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string args)
        {
            var arena = player.Manager.Worlds.Values
                .OfType<DeathArena>()
                .FirstOrDefault(w => !w.Deleted && w.CurrentState != DeathArena.ArenaState.Ended);
            if (arena == null)
            {
                player.SendError("Oryx's Arena is unavailable.");
                return false;
            }

            player.Reconnect(arena);
            return true;
        }
    }
  ...
  */
    ...
    class LeaderboardCommand : Command { public LeaderboardCommand():base("leaderboard",alias:"lb"){ } protected override bool Process(Player p,RealmTime t,string a){p.SendInfo(EclipseLeaderboards.Describe(p.Client.Account,string.IsNullOrWhiteSpace(a)?"weekly-clears":a.Trim()));return true;} }
    class GuildProgressCommand : Command { public GuildProgressCommand():base("guildprogress",alias:"gtrophy"){ } protected override bool Process(Player p,RealmTime t,string a){p.SendInfo(GuildProgressionService.Describe(p.Client.Account));return true;} }
