        ...
        protected override bool Process(Player player, RealmTime time, string args)
        {
            ...
            if (args.ContainsIgnoreCase("Oryx Arena"))
            {
                if (player.Owner is Nexus)
                {
                    ...
                    if (arena == null)
                    {
                        arena = (DeathArena)player.Manager.AddWorld(new DeathArena(
                            player.Manager.Resources.Worlds["DeathArena"]));
                    }

                    var portal = new Portal(player.Manager, 0x441c, null)
                    {
                        Name = "Oryx's Arena Portal",
                        WorldInstance = arena
                    };

                    if (!player.Manager.Monitor.AddPortal(arena.Id, portal,
                            new Position { X = 334.5f, Y = 148.5f }))
                    {
                        player.SendError("Oryx's Arena is already open.");
                        return false;
                    }

                    player.Manager.Chat.Announce($"{player.Name} has hosted an Oryx's Arena event!");
                    return true;
                }
                else
                {
