...
public class TextHandler {
    ...
    public function addTextAsTextLine(_arg1:Text):void {
        ...
        // System text is ordered after UPDATEACK by the server.  Preserve a valid
        // message if a reconnect still races the new player model: it is displayed,
        // but cannot be classified as a tell until the player exists.
        _local2.isToMe = (((this.model != null) && ((this.model.player != null))) ? (_arg1.recipient_ == this.model.player.name_) : false);
        if (((this.model == null) || ((this.model.player == null)))) {
            trace("[TEXT_CLIENT_TRACE] chat fallback objectId=" + _arg1.objectId_ + " sender=" + _arg1.name_ + " recipient=" + _arg1.recipient_ + " stars=" + _arg1.numStars_ + " bubble=" + _arg1.bubbleTime_ + " nameColor=" + _arg1.nameColor_ + " textColor=" + _arg1.textColor_ + " text=" + _arg1.text_);
        }
    ...
    private function showSpeechBaloon(_arg1:Text, _arg2:String):void {
        ...
        var _local8:Boolean = (((this.model != null) && ((this.model.gameObjects != null))));
        var _local9:Boolean = (((this.model != null) && ((this.model.player != null))));
        trace("[TEXT_CLIENT_TRACE] objectId=" + _arg1.objectId_ + " sender=" + _arg1.name_ + " bubble=" + _arg1.bubbleTime_ + " objectMap=" + _local8 + " player=" + _local9 + " text=" + _arg2);
        // Entity speech can arrive while the map is being replaced during a reconnect.
        // Keep the message in the chat log (execute continues after this method), but do
        // not attempt a speech balloon until the target map has created its object map.
        if ((!(_local8)) || (!(_local9))) {
            trace("[TEXT_CLIENT_TRACE] speech balloon deferred: map/player is not ready.");
            return;
        }
        var _local3:GameObject = this.model.getGameObject(_arg1.objectId_);
        trace("[TEXT_CLIENT_TRACE] objectId=" + _arg1.objectId_ + " objectFound=" + (_local3 != null));
        if (_local3 != null) {
